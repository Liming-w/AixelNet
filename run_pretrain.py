...
from AixelNet.load_pretrain_data import load_all_label_data_for_pretrain
...
def parse_args():
    ...
    parser.add_argument(
        "--label_data_args",
        type=str,
        default="dataset/pretrain/",
        help="Pre-training data path",
    )
    parser.add_argument("--save_model", type=str, default="./AixelNet-pretrained", help="Path to save the model")
    parser.add_argument("--num_data", type=int, default=1000, help="Number of pretrain datasets to use")
    parser.add_argument("--log_path", type=str, default="./pretrain.log", help="Log file path")

    parser.add_argument("--hidden_dim", type=int, default=128, help="Hidden dimension")
    parser.add_argument("--wo_arg", type=str, choices=['wo_meta', 'wo_hypernetwork', 'wo_sparse', 'wo_regularization'], default=None, help="Ablation setting")
    parser.add_argument("--hyper_arg", type=str, choices=['None', 'Uniform', 'Heuristic'], default=None, help="Hypernetwork setting")
    ...
    parser.add_argument("--num_k_model", type=int, default=6, help="Number of ensemble models")
...
log_parent = Path(_args.log_path).parent
if str(log_parent) != ".":
...
trainset, valset, cat_cols, num_cols, bin_cols, dataset_paths = load_all_label_data_for_pretrain(
    label_data_path=_args.label_data_args,
    seed=42,
...
if not dataset_paths:
    raise ValueError("No valid dataset paths found, please check if 'label_data_args' is correct.")

num_classes_list = []
for train in trainset:
    labels = train[0][1]
    num_classes = len(set(labels)) if labels is not None else 2
