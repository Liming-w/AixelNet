...
from .evaluator import get_eval_metric_fn, EarlyStopping, evaluate, predict_new
...
class Trainer:
    def __init__(self,
        ...
        **kwargs,
        ...
        for dataindex, trainset in enumerate(train_set_list):
            (X, y, meta_features), table_flag = trainset
            inputs = self.feature_extractor(X, table_flag=table_flag)
            new_train_list.append(((inputs, y, meta_features), table_flag))

        # Create DataLoader for each train set
        self.trainloader_list = [
            torch.utils.data.DataLoader(
                TrainDataset(trainset),  # trainset: ((inputs, y, meta_features), table_flag)
        ...
        if test_set_list is not None:
            for dataindex, testset in enumerate(test_set_list):
                (Xv, yv, meta_features_v), tfv = testset
                inputs_val = self.feature_extractor(Xv, table_flag=tfv)
                new_test_list.append(((inputs_val, yv, meta_features_v), tfv))
    ...
    def train(self, eval_data=None):
        ...
        for epoch in trange(args['num_epoch'], desc='Epoch'):
            ...
            for dataindex, trainloader in enumerate(self.trainloader_list):
                for batch_idx, batch in enumerate(trainloader):
                    # Batch contains: inputs, y, meta_features, table_flag
                    inputs, y, meta_features, table_flag = batch
                    ...
                    # Prefer precomputed meta features; keep DataFrame fallback for old callers.
                    if isinstance(meta_features, pd.DataFrame):
                        logits, loss = self.model(
                            x_data, y, df=meta_features, table_flag=table_flag,
                            sparse=False, M=2, lambda1=self.lambda1, lambda2=self.lambda2
                        )
                    else:
                        logits, loss = self.model(
                            x_data, y, table_flag=table_flag, meta_features=meta_features,
                            sparse=False, M=2, lambda1=self.lambda1, lambda2=self.lambda2
                        )
                    loss.backward()
                    # gradient clipping
            ...
            if self.testloader_list is not None:
                ...
                if self.early_stopping(-eval_res, self.model) and eval_data:
                    eval_meta = eval_data[0][2]
                    eval_df = eval_meta if isinstance(eval_meta, pd.DataFrame) else None
                    eval_meta_features = None if isinstance(eval_meta, pd.DataFrame) else eval_meta
                    if self.regression_task:
                        ypred = predict_new(
                            self.model, eval_data[0][0], df_test=eval_df,
                            meta_features=eval_meta_features, regression_task=True
                        )
                        ans = evaluate(ypred, eval_data[0][1], metric='rmse')
                    else:
                        ypred = predict_new(
                            self.model, eval_data[0][0], df_test=eval_df,
                            meta_features=eval_meta_features
                        )
                ...
                if self.early_stopping.early_stop:
                    ...
                    break
                logger.info('Epoch: {}, Train Loss: {:.4f}, Test {}: {:.6f}, Time: {:.1f} secs'.format(
                    epoch, train_loss_all, self.args['eval_metric_name'], eval_res, time.time()-start_time))
            else:
                logger.info('Epoch: {}, Train Loss: {:.4f}, Time: {:.1f} secs'.format(
                    epoch, train_loss_all, time.time()-start_time))
        ...
        logger.info('Training complete, time: {:.1f} secs.'.format(time.time()-start_time))
