...
def predict_new(clf, 
    ...
    for i in range(0, x_len, eval_batch_size):
        end = min(i + eval_batch_size, x_len)
        # Prepare the input batch
        x_cat_input_ids = x_test['x_cat_input_ids'][i:end] if x_test['x_cat_input_ids'] is not None else None
        x_cat_att_mask = x_test.get('x_cat_att_mask', None)[i:end] if x_test.get('x_cat_att_mask', None) is not None else None
        col_cat_input_ids = x_test.get('col_cat_input_ids', None)
        col_cat_att_mask = x_test.get('col_cat_att_mask', None)
        x_num = x_test['x_num'][i:end] if x_test['x_num'] is not None else None
        num_col_input_ids = x_test['num_col_input_ids']
        num_att_mask = x_test.get('num_att_mask', None)
        y_batch = None
        if y_test is not None:
            y_batch = y_test.iloc[i:end] if hasattr(y_test, "iloc") else y_test[i:end]
        ...
        with torch.no_grad():
            logits, loss = clf(
                bs_x_test, y_batch, df=df_test, table_flag=table_flag,
        ...
        elif logits.dim() == 1 or logits.shape[-1] == 1:  # binary classification
    ...
    if bootstrap:
        for i in range(10):
            ...
            try:
                ...
                logger.warning("Bootstrap evaluation failed.")
                continue
